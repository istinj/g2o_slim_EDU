...
void SparseMatrix::setElement(const int r_, const int c_, const real_ value_){
   if (r_ >= _num_rows || c_ >= _num_cols){
      cerr << RED << "Out of bound" << RESET << endl;
      return;
   }

   IntSparseMatrixBlockPtrMap& current_row = _row_container[r_];
   IntSparseMatrixBlockPtrMap::iterator it = current_row.find(c_);
...
bool SparseMatrix::isNonZeroElement(const int r_, const int c_) const {
   const IntSparseMatrixBlockPtrMap& curr_row = _row_container[r_];
   IntSparseMatrixBlockPtrMap::const_iterator it = curr_row.find(c_);
   if(it == curr_row.end())
      return false;
   return true;
}

real_ SparseMatrix::getElement(const int r_, const int c_) const{
   if (r_ >= _num_rows || c_ >= _num_cols){
      cerr << RED << "Out of bound" << RESET << endl;
      return 0.0;
   }
...
void SparseMatrix::printElement(const int r_, const int c_) const {
   IntSparseMatrixBlockPtrMap current_row = _row_container[r_]; //! No copy through reference
   real_ value = getElement(r_, c_);
...
bool SparseMatrix::evaluateScalarProdStructure(const IntSparseMatrixBlockPtrMap& row_1_,
      const IntSparseMatrixBlockPtrMap& row_2_, int max_pos_){
   IntSparseMatrixBlockPtrMap::const_iterator it_1 = row_1_.begin();
   IntSparseMatrixBlockPtrMap::const_iterator it_2 = row_2_.begin();
...
real_ SparseMatrix::scalarProd(const IntSparseMatrixBlockPtrMap& row_1_,
...
void SparseMatrix::evaluateCholeskyStructure(SparseMatrix& cholesky_){
   ...
   for (int r = 0; r < _num_rows; ++r) {
      const IntSparseMatrixBlockPtrMap& current_row = _row_container[r];
      IntSparseMatrixBlockPtrMap& chol_current_row = cholesky_._row_container[r];
      ...
      for(int c = starting_col_idx; c <= r; ++c){
         ...
         if(isNonZeroElement(r,c)){
            ...
            IntSparseMatrixBlockPtrMap& chol_upper_row = cholesky_._row_container[c];
...
void SparseMatrix::cholesky(SparseMatrix& cholesky_){
   ...
   for (int r = 0; r < _num_rows; ++r) {
      const IntSparseMatrixBlockPtrMap& current_row = _row_container[r];
      IntSparseMatrixBlockPtrMap& chol_current_row = cholesky_._row_container[r];
      real_ diagonal_values[_num_rows];
      real_ inverse_diagonal_values[_num_rows];
      if(current_row.empty())
         return;
      int starting_col_idx = current_row.begin()->first;
      for (int c = starting_col_idx; c <= r; ++c) {
         real_ accumulator = 0.0;
         IntSparseMatrixBlockPtrMap& chol_upper_row = cholesky_._row_container[c];
         accumulator = getElement(r, c) - scalarProd(chol_current_row, chol_upper_row, c-1);
         real_ chol_curr_rc_value = 0.0;
         if (r == c) {
            if (accumulator > 0) {
