...
template<typename BlockType_>
void SparseBlockMatrix<BlockType_>::exportToTxt(const std::string& filename_){
	ofstream file(filename_);
	if(!file.is_open())
		throw std::runtime_error("Error in opening the file!");

	cerr << "Exporting a (" << _num_block_rows << " x " << _num_block_cols << ") matrix..." << endl;
	for (int r = 0; r < _num_block_rows; ++r) {
		for (int c = 0; c < _num_block_cols; ++c) {
			DenseBlock block = getBlock(r,c);
			file << block << endl;
		}
	}
	cerr << "DONE!" << endl;
...
bool SparseBlockMatrix<BlockType_>::solveLinearSystem(const DenseVector<VectorBlockType_>& RHS_Vector_,
	...
	for (int r = 0; r < _num_block_rows; ++r) {
		for (int c = 0; c < _num_block_cols; ++c) {
			...
			L.printBlock(r,c);
			cerr << BLUE << "U block" << RESET << endl;
			U.printBlock(r,c);
			cerr << GREEN << "B block" << RESET << endl;
			RHS_Vector_.printBlock(r);
...
template<typename BlockType_>
void SparseBlockMatrix<BlockType_>::addDamping(const real_ damping_){
